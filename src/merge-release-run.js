...
const publishAccess = process.env.PUBLISH_ACCESS || 'public';
const publishScriptKey = process.env.PUBLISH_SCRIPT_KEY || 'publish';
...
const run = async () => {
  if (!process.env.NPM_AUTH_TOKEN)
    throw new Error('npmpublishmanager requires NPM_AUTH_TOKEN');
  let latest;
  try {
  ...
  if (publishScriptKey) {
    exec(`npm run ${publishScriptKey} --access ${publishAccess}`, deployDir);
  } else {
    exec(`npm publish --access ${publishAccess}`, deployDir);
  }
